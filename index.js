...
function findMinMax([firstElement, ...restElements]) {
	...
	return console.log("Min =", minValue, "Max =", maxValue);
...
function addTenToEach(array) {
	let arrayAddTen = [];
	for (const index in array) {
		arrayAddTen[index] = array[index] + 10;
	}
	return console.log(arrayAddTen);
...
console.log(swapMinMax([1, 2, 3, 4, 5, 6]));
...
function getEvenNumbers(arr) {
	...
	for (const index in arr) {
		if (arr[index] % 2 == 0) {
			arrEven[arrEven.length] = arr[index];
		}
	}
	return arrEven;
...
function multiplyByIndex(arr) {
	let arrMultiply = [];
	for (const index in arr) {
		arrMultiply[arrMultiply.length] = arr[index] * index;
	}
	return arrMultiply;
	// Створюємо пустий масив для зберігання результату
	// Проходимо через кожен елемент вхідного масиву
...
function replaceNumbers(arr) {
	...
	for (const item of arr) {
		if (item <= 10) {
			arrReplace[arrReplace.length] = "Less than or equal to 10";
		} else if (item >= 10) {
			arrReplace[arrReplace.length] = "Greater than 10";
		}
	}
	return arrReplace;
	// Створюємо пустий масив для зберігання результату
